...
class CircularBufferTest : public ::testing::Test, public CircularBufferTestBase
{
protected:
    void SetUp() override
    {
...
TEST_F(CircularBufferTest, FillConstruction)
{
    circular_buffer<int, 5> buffer(42);
...
TEST_F(CircularBufferTest, UncheckedPushOperations)
{
    ...
    buffer.push_back_unchecked(1);
    buffer.push_back_unchecked(2);
    buffer.push_back_unchecked(3);
    
    EXPECT_EQ(buffer.size(), 3);
    EXPECT_TRUE(buffer.full());
    
    // This should overwrite (no return value to check)
    buffer.push_back_unchecked(4);
...
{
    circular_buffer<int, 3, uint32_t, alignof(int), 64, overflow_policy::overwrite> buffer;
...
{
    circular_buffer<int, 3, uint32_t, alignof(int), 64, overflow_policy::discard> buffer;
...
TEST_F(CircularBufferTest, PopBackSafe)
{
    circular_buffer<int, 5> buffer{1, 2, 3};
    
    auto result1 = buffer.pop_back_safe();
    ASSERT_TRUE(result1.has_value());
    EXPECT_EQ(result1.value(), 3);
    EXPECT_EQ(buffer.size(), 2);
    
    auto result2 = buffer.pop_back_safe();
    ASSERT_TRUE(result2.has_value());
    EXPECT_EQ(result2.value(), 2);
    EXPECT_EQ(buffer.size(), 1);
    
    auto result3 = buffer.pop_back_safe();
    ASSERT_TRUE(result3.has_value());
    EXPECT_EQ(result3.value(), 1);
    EXPECT_EQ(buffer.size(), 0);
    
    // Should return nullopt when empty
    auto result4 = buffer.pop_back_safe();
    EXPECT_FALSE(result4.has_value());
}

TEST_F(CircularBufferTest, PopFrontSafe)
{
    circular_buffer<int, 5> buffer{1, 2, 3};
    
    auto result1 = buffer.pop_front_safe();
    ASSERT_TRUE(result1.has_value());
    ...
    // Should return nullopt when empty
    auto result2 = buffer.pop_front_safe();
    EXPECT_FALSE(result2.has_value());
}

TEST_F(CircularBufferTest, PopConvenienceMethod)
{
    circular_buffer<int, 5> buffer{1, 2, 3};
    
    auto result = buffer.pop();  // Should call pop_back_safe()
...
TEST_F(CircularBufferTest, DifferentIndexTypes)
{
    ...
    circular_buffer<int, 10, uint8_t> buffer8;
    ...
    circular_buffer<int, 1000, uint16_t> buffer16;
    EXPECT_EQ(buffer16.capacity(), 1000);
    
    // Test with uint64_t indices  
    circular_buffer<int, 100, uint64_t> buffer64;
...
TEST_F(CircularBufferTest, EmbeddedStorageDetection)
{
    ...
    circular_buffer<int, 32, uint32_t, alignof(int), 64> small_buffer;
    EXPECT_TRUE(small_buffer.uses_embedded());
    
    // Large buffer should use heap storage
    circular_buffer<int, 128, uint32_t, alignof(int), 64> large_buffer;
    EXPECT_FALSE(large_buffer.uses_embedded());
}

TEST_F(CircularBufferTest, CustomEmbeddedThreshold)
{
    // Force heap allocation with low threshold
    circular_buffer<int, 32, uint32_t, alignof(int), 16> heap_buffer;
    EXPECT_FALSE(heap_buffer.uses_embedded());
    
    // Force embedded storage with high threshold
    circular_buffer<int, 128, uint32_t, alignof(int), 256> embedded_buffer;
    EXPECT_TRUE(embedded_buffer.uses_embedded());
}
...
TEST_F(CircularBufferTest, PushBackRange)
{
    circular_buffer<int, 10> buffer;
    std::vector<int> data = {1, 2, 3, 4, 5};
    
    auto result = buffer.push_back_range(data.begin(), data.end());
    
    EXPECT_EQ(result.inserted_count, 5);
    EXPECT_EQ(result.overwritten_count, 0);
    EXPECT_EQ(result.discarded_count, 0);
    EXPECT_TRUE(verify_buffer_contents(buffer, data));
}

TEST_F(CircularBufferTest, PushBackRangeWithOverwrite)
{
    circular_buffer<int, 3, uint32_t, alignof(int), 64, overflow_policy::overwrite> buffer;
    std::vector<int> data = {1, 2, 3, 4, 5};
    ...
    EXPECT_EQ(result.inserted_count, 3);
    EXPECT_EQ(result.overwritten_count, 2);
    EXPECT_EQ(result.discarded_count, 0);
    EXPECT_EQ(buffer.size(), 3);
...
TEST_F(CircularBufferTest, PushBackRangeWithDiscard)
{
    circular_buffer<int, 3, uint32_t, alignof(int), 64, overflow_policy::discard> buffer;
    std::vector<int> data = {1, 2, 3, 4, 5};
    ...
    EXPECT_EQ(result.inserted_count, 3);
    EXPECT_EQ(result.overwritten_count, 0);
    EXPECT_EQ(result.discarded_count, 2);
    EXPECT_EQ(buffer.size(), 3);
...
TEST_F(CircularBufferTest, EmptyBufferBehavior)
{
    ...
    EXPECT_FALSE(buffer.pop_back_safe().has_value());
    EXPECT_FALSE(buffer.pop_front_safe().has_value());
